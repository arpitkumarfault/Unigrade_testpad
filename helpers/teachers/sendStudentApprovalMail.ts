...
export default async function sendStudentApprovedEmail({
  to,
  studentName,
  studentId,
  universityCode,
  department,
}: StudentApprovedEmailArgs) {
  const loginUrl = process.env.LOGIN_URL || 'https://unigrade-testpad.com/studentlogin'
  ...
  <div style="font-family: Arial, sans-serif; line-height:1.6; color:#333; padding:20px; max-width:600px; margin:auto; border:1px solid #e0e0e0; border-radius:8px; background-color:#f9f9f9;">
    ...
    <div style="background-color:#ffffff; border:1px solid #ddd; border-radius:6px; padding:16px; margin:15px 0;">
      <table style="width:100%; border-collapse:collapse;">
        <tr>
          <td style="padding:10px 0; color:#555; font-weight:bold; width:45%;">Student Name:</td>
          <td style="padding:10px 0; color:#2c3e50;">${studentName}</td>
        </tr>
        ${studentId ? `
        <tr>
          <td style="padding:10px 0; color:#555; font-weight:bold;">Student ID:</td>
          <td style="padding:10px 0; color:#16a085; font-family:monospace;">${studentId}</td>
        </tr>
        ` : ''}
        <tr>
    ...
    <ul style="line-height:1.8; color:#555; padding-left:20px;">
      ...
      <li>Participate in scheduled exams</li>
      <li>View your grades and progress reports</li>
