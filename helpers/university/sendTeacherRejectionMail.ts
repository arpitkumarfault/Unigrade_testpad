...
export default async function sendTeacherRejectedEmail({
  to,
  teacherName,
  universityCode,
  department,
  reason,
}: TeacherRejectedEmailArgs) {
  const supportEmail = process.env.SUPPORT_EMAIL || 'support@unigrade-testpad.com'
  const registrationUrl = process.env.REGISTRATION_URL || 'https://unigrade-testpad.com/teacher/register'
